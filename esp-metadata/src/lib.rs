...
impl Config {
    ...
    fn validate(&self) -> Result<()> {
        ...
        dma::validate(self.peripherals(), &self.device.peri_config)?;
        Ok(())
    }
...
{limitations}
        ...
        let dma_peripherals = dma_peripherals
            ...
            .map(|(name, dma_peripheral)| {
                ...
                let variant_name = dma::peripheral_dma_variant_ident(name);
                quote! { #peri, #variant_name, #dma_peripheral }
            })
            .collect::<Vec<_>>();

        let dma_channel_rows = dma::sorted_dma_channel_rows(self.peripherals());
