...
impl Debug for Instant {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        write!(f, "Instant({} µs since epoch)", self.micros_since_epoch())
    }
}

impl Display for Instant {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        write!(f, "{} µs since epoch", self.micros_since_epoch())
    }
}
...
impl defmt::Format for Instant {
    fn format(&self, f: defmt::Formatter<'_>) {
        defmt::write!(f, "{=u64} µs since epoch", self.micros_since_epoch())
    }
}
...
impl Instant {
    ...
    /// Returns the elapsed time since boot in microseconds.
    pub const fn micros_since_epoch(&self) -> u64 {
        self.0.ticks()
    }

    /// Returns the elapsed time since boot in milliseconds.
    pub const fn millis_since_epoch(&self) -> u64 {
        self.micros_since_epoch() / 1_000
    }

    /// Returns the elapsed time since boot in seconds.
    pub const fn seconds_since_epoch(&self) -> u64 {
        self.micros_since_epoch() / 1_000_000
    }
